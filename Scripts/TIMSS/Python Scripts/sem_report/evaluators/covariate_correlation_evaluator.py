...
class CovariateCorrelationEvaluator:
    ...
    Computes:
      ...
      - Heatmap of mean |Δr| for covariate → ACM correlations (TIMSS)
    ...
    def _absolute_delta_long(self) -> pd.DataFrame:
        ...
        # discover REAL covariate columns
        real_cols = [c for c in df.columns if c.startswith("REAL__")]
        covariates = [c.replace("REAL__", "") for c in real_cols]

        # discover techniques
        techniques = {
            c.split("__", 1)[0]
            for c in df.columns
            if "__" in c and not c.startswith("REAL__")
        }

        for i, var_i in enumerate(variables):
            for j, var_j in enumerate(covariates):
                if var_i == var_j:
                    continue  # skip diagonal

                real_col = f"REAL__{var_j}"
                if real_col not in df.columns:
                    continue

                real_vals = df.loc[df["Variable"] == var_i, real_col]
                if real_vals.empty:
                    continue

                real = real_vals.values[0]
                if pd.isna(real):
                    continue

                for tech in techniques:
                    synth_col = f"{tech}__{var_j}"
                    if synth_col not in df.columns:
                        continue

                    synth = df.loc[df["Variable"] == var_i, synth_col].values[0]
                    if pd.isna(synth):
                        continue

                    records.append({
                        "Technique": tech,
                        "Variable": f"{var_i}–{var_j}",
        ...
        if out.empty:
            raise RuntimeError(
                ...
                "REAL__* columns not found or empty."
            )
    ...
    def _plot_delta_heatmap(self, heat_df: pd.DataFrame):
        ...
        ax.set_title("Mean |Δr| vs REAL (ACM covariate correlations)")
