...
class StructuralPathEvaluator:
    ...
    def run(self):
        ...
        beta_table = self._build_beta_table(mean_df)
        export_table(
            beta_table,
    ...
    def plot_bar_grid(self, df: pd.DataFrame):
        ...
        for i, (_, r) in enumerate(df.iterrows()):
            ...
            values = [r["REAL__Std_B"]] + [
                r[f"{tech}__Std_B"] for tech in self.techniques
            ]
    ...
    def plot_interval_grid(self, mean_df: pd.DataFrame, range_df: pd.DataFrame):
        ...
        for i, (_, r_mean) in enumerate(mean_df.iterrows()):
            ...
            for j, tech in enumerate(self.techniques):
                mean = r_mean[f"{tech}__Std_B"]
                sd = r_range[f"{tech}__Bootstrap_SD_max"]
    ...
    def _build_beta_table(self, df: pd.DataFrame) -> pd.DataFrame:
        rows = []
        ...
        for _, r in df.iterrows():
            ...
            for tech in self.techniques:
                row[tech] = self._format_beta_with_sig(
                    r[f"{tech}__Std_B"],
                    r[f"{tech}__Std_p"],
                )
    ...
    def _direction_and_rank_summary(self, df: pd.DataFrame) -> pd.DataFrame:
        """
        Combined directional consistency and rank preservation summary.

        Directional consistency =
            (# paths with correct sign) / (total # paths)

        Rank preservation =
            Spearman correlation of |β| ranks vs REAL across all paths
        """
        rows = []
        ...
        for tech in self.techniques:
            synth_beta = df[f"{tech}__Std_B"]

            # ----------------------------
            # Directional consistency
            # ----------------------------
            correct = (
                np.sign(synth_beta) == np.sign(real_beta)
            ).sum()
            ...
            rows.append({
                "Technique": tech,
                "Directional_Consistency": f"{directional_consistency:.3f}",
                "Rank_Preservation_ρ": round(rho, 3) if pd.notna(rho) else np.nan,
            })
    ...
    def _range_overlap(
        ...
        for _, r in mean_df.iterrows():
            ...
            row = {"Path": r["Path"], "REAL β": r["REAL__Std_B"]}

            r_range = range_df.loc[range_df.Path == r["Path"]].iloc[0]

            for tech in self.techniques:
                mean = r[f"{tech}__Std_B"]
                sd = r_range[f"{tech}__Bootstrap_SD_max"]
    ...
    def _plot_bar(self, mean_df: pd.DataFrame, path: str):
        ...
        for tech in self.techniques:
            labels.append(tech)
            values.append(r[f"{tech}__Std_B"])
