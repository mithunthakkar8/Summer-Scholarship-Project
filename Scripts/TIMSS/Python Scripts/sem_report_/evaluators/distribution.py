...
class DistributionEvaluator:
    ...
    def run(self):
        ...
        for tech, path in self.technique_paths.items():
            ...
            logger.info(f"[Distribution] Processing technique: {tech}")
            csvs = self._load_csvs(tech, path)
            logger.info(f"[Distribution] {tech}: {len(csvs)} CSV(s)")
            ...
            if df_syn_use.empty or df_real_use.empty:
                logger.warning(f"[Distribution] {tech}: No data to process after loading CSVs. Skipping.")
                continue
            ...
            logger.info(f"[Distribution] {tech}: Plots generated")
        ...
        df_dist = self._sort_by_variable_then_technique(pd.DataFrame(dist_rows))

        # Sample size
        export_table(df_n, path=self.tables_dir   / "table_sample_size", config=self.export_config, index=False)

        # Continuous summary
        export_table(df_cont, path=self.tables_dir   / "table_continuous_summary", config=self.export_config, index=False)

        # Boundary adherence
        export_table(df_bound, path=self.tables_dir   / "table_boundary_adherence", config=self.export_config, index=False)

        # Missingness diagnostics
        export_table(df_miss, path=self.tables_dir   / "table_missingness_diagnostics", config=self.export_config, index=False)

        # Range sanity vs REAL
        export_table(df_range, path=self.tables_dir   / "table_range_sanity_vs_real", config=self.export_config, index=False)
        ...
        export_table(df_cat, path=self.appendix_dir / "table_category_adherence", config=self.export_config, index=False)

        # Tables version (no proportions)
        df_cat_main = df_cat.drop(columns=["Category_Proportions"], errors="ignore")
        export_table(df_cat_main, path=self.tables_dir / "table_category_adherence", config=self.export_config, index=False)
    ...
    def _load_csvs(self, tech: str, path: Path) -> list[Path]:
        logger.debug(f"Loading CSVs for {tech} from {path}")
        ...
        csvs = [
            Path(f) for f in glob.glob(str(path / "*.csv"))
            if "correction" not in f.lower()
        ]

        if not csvs:
            logger.error(f"No CSVs found for {tech} in {path}")
            # raise FileNotFoundError(f"No CSVs found for {tech} in {path}")

        logger.debug(f"Found CSVs for {tech}: {[c.name for c in csvs]}")
        return csvs
    ...
    def _build_boundary_adherence_table(self, df_real, df_syn, technique):
        ...
        for col in self.numeric_cols:
            ...
            real_mean_sd = f"{r.mean():.3f} ± {r.std():.3f}" if r.notna().any() else ""
            real_range = f"{int(r.min())}–{int(r.max())}" if r.notna().any() else ""

            syn_mean_sd = f"{s.mean():.3f} ± {s.std():.3f}" if s.notna().any() else ""
            syn_range = f"{int(s.min())}–{int(s.max())}" if s.notna().any() else ""

            # Boundary adherence is not meaningful for REAL row; set NA
            ba = (
                np.nan
                if technique == "REAL"
                else self._boundary_adherence(r, s)
            )
            ...
            if technique == "REAL":
                rows.append({
                    "Variable": col,
                    "Technique": "REAL",
                    "Mean ± SD": real_mean_sd,
                    "Range": real_range,
                    "Boundary_Adherence": np.nan,
                })
            else:
                rows.append({
                    "Variable": col,
