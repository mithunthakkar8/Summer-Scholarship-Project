...
class CovariateCorrelationEvaluator:
    ...
    def _format_range_summary(self, ranges: pd.DataFrame) -> pd.DataFrame:
        """
        Wide table:
        Variable | REAL | TECH1 | TECH2 | ...
        Each TECH cell: [lo–hi] ✓/✗
        """
        rows = []
        ...
        for (var, tgt), g in ranges.groupby(["Variable", "Target"]):
            row = {
                "Variable": f"{var} → {tgt}",
                "REAL": round(g["REAL"].iloc[0], 3)
                if pd.notna(g["REAL"].iloc[0]) else np.nan
            }
            ...
            for _, r in g.iterrows():
                ...
                if pd.notna(lo) and pd.notna(hi):
                    ...
                    mark = "✓" if ok else "✗"
                    row[r["Technique"]] = f"{interval} {mark}"
                else:
                    row[r["Technique"]] = ""
    ...
    def run(self):
        ...
        heat = self._heatmap_matrix(long_df)
        ...
        export_table(
            range_summary,
            self.tables_dir / "table_covariate_corr_ranges.csv",
        ...
        export_table(
            long_df,
            self.appendix_dir / "covariate_delta_long.csv",
            config=self.export_config,
            index=False,
        )
        ...
        export_table(
            summary,
            self.tables_dir / "table_covariate_corr_error.csv",
            config=self.export_config,
            index=False,
        )
        ...
        export_table(
            heat,
            self.appendix_dir / "covariate_delta_heatmap_matrix.csv",
            config=self.export_config,
            index=True,
        )
        ...
        export_table(
            ranges,
            self.appendix_dir / "covariate_corr_ranges_long.csv",
            config=self.export_config,
            index=False,
        )
