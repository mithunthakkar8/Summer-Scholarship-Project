...
for (seed in seeds) {
  CNT_Model <- glue("SGP_CTGAN_seed{seed}")
  cat("\n==============================\n")
  cat("Running SEM for seed:", seed, "\n")
  cat("==============================\n")
  df <- read.csv(
    file.path(DATA_DIR, glue("synthetic_ctgan_seed_{CNT}_{seed}.csv"))
  )
  ...
  write_csv(
    missing_summary,
    file.path(DATA_DIR, glue("sem_missing_summary_{CNT_Model}.csv"))
  )
  ...
  if (!is.null(indirect_spi_sms_smp)) {
    ...
    write_csv(
      indirect_tbl,
      file.path(DATA_DIR, glue("sem_full_indirect_SPI_SMS_SMP_{CNT_Model}.csv"))
    )
  }
  ...
  if (!is.null(boot_sum$bootstrapped_paths)) {
    ...
    if (nrow(structural_paths) > 0) {
      ...
      write_csv(
        paths_std,
        file.path(DATA_DIR, glue("sem_full_std_paths_{CNT_Model}.csv"))
      )
      
      write_csv(
        structural_paths,
        file.path(DATA_DIR, glue("sem_full_unstd_paths_{CNT_Model}.csv"))
      )
    }
  }
  ...
  if (!is.null(sum_pls$paths)) {
    ...
    write_csv(
      r_squared,
      file.path(DATA_DIR, glue("sem_full_rsquared_{CNT_Model}.csv"))
    )
  }
  ...
  if (!is.null(boot_sum$bootstrapped_total_paths)) {
    ...
    write_csv(
      total_effects,
      file.path(DATA_DIR, glue("sem_full_total_effects_{CNT_Model}.csv"))
    )
  }
  ...
  if (!is.null(srmr_vals)) {
    ...
    write_csv(
      srmr_df,
      file.path(DATA_DIR, glue("sem_full_pls_srmr_{CNT_Model}.csv"))
    )
  }
  ...
  if (!is.null(sum_pls$reliability)) {
    ...
    write_csv(
      reliability_metrics,
      file.path(DATA_DIR, glue("sem_full_reliability_metrics_labeled_{CNT_Model}.csv"))
    )
  ...
  if (!is.null(sum_pls$validity$fl_criteria)) {
    ...
    write_csv(
      fl_df,
      file.path(DATA_DIR, glue("sem_full_fornell_larcker_{CNT_Model}.csv"))
    )
  }
  ...
  if (!is.null(sum_pls$validity$htmt)) {
    ...
    write_csv(
      htmt_df,
      file.path(DATA_DIR, glue("sem_full_htmt_{CNT_Model}.csv"))
    )
  }
  ...
  # ------------------------------------------------
  mapping <- read.csv(file.path(DATA_DIR, "pisa_variable_mapping.csv"), stringsAsFactors = FALSE)
  
  # Create a named vector: names = original code, values = safe_short
  name_map <- setNames(mapping$safe_short, mapping$code)
  ...
  output_file <- file.path(
    DATA_DIR,
    glue("sem_full_dataset_raw_plus_selected_latent_scores_{CNT_Model}.csv")
  )
  ...
  # write_csv(
  #   struct_resid,
  #   file.path(DATA_DIR, glue("sem_full_structural_residuals_SMS_SMP_{CNT_Model}.csv"))
  # )
  ...
  if (all(c("SMP","SMS","SPI") %in% colnames(latent_scores))) {
    ...
    write_csv(
      latent_corr_df,
      file.path(DATA_DIR, glue("sem_full_latent_correlations_SMP_SMS_SPI_{CNT_Model}.csv"))
    )
  }
  ...
  write_csv(
    clean_loadings,
    file.path(DATA_DIR, glue("sem_full_indicator_loadings_R2_clean_{CNT_Model}.csv"))
  )
  ...
  if (!is.null(paths_std)) {
    ...
    if (!any(is.na(c(a_std, b_std, cprime_std)))) {
      ...
      write_csv(
        med_df,
        file.path(DATA_DIR, glue("sem_full_mediation_SPI_SMS_SMP_{CNT_Model}.csv"))
      )
    }
  }
