...
# CONFIG
# ================================================================
DATA_DIR <- "C:/Users/mithu/Documents/MEGA/VUW/Summer Research Project/Datasets/PISA 2022/Great Baseline"
MODEL_BASE <- "DistilGPT2_Baseline"
seeds <- c(42, 101, 202, 303, 404)

# Helper to build model tag
seed_tag <- function(seed) glue("{MODEL_BASE}_seed_{seed}")
...
write_csv(paths_summary,
          file.path(DATA_DIR, glue("agg_std_paths_{MODEL_BASE}.csv")))
...
write_csv(r2_summary,
          file.path(DATA_DIR, glue("agg_r_squared_{MODEL_BASE}.csv")))
...
write_csv(rel_summary,
          file.path(DATA_DIR, glue("agg_reliability_{MODEL_BASE}.csv")))
...
write_csv(htmt_summary,
          file.path(DATA_DIR, glue("agg_htmt_{MODEL_BASE}.csv")))
...
write_csv(indirect_summary,
          file.path(DATA_DIR, glue("agg_indirect_SPI_SMS_SMP_{MODEL_BASE}.csv")))
