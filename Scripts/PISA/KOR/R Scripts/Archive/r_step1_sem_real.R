...
# ============================
df <- read.csv(file.path(DATA_DIR, "df_core.csv"))
...
a  <- coef_of("SMS", "SPI")
c1 <- coef_of("SMS", "female")
c2 <- coef_of("SMS", "ESCS_z")
c3 <- coef_of("SMS", "ST001D01T")
c4 <- coef_of("SMS", "MISCED")
c5 <- coef_of("SMS", "SCHSIZE_z")

# Intercept for SMS (lavaan estimates it by default)
int_SMS <- pe$est[pe$lhs == "SMS" & pe$op == "~1"]

# 3B-2: Compute predicted SMS (latent-level regression prediction)
pred_SMS <- int_SMS +
  a  * df_latent$SPI +
  c1 * df_latent$female +
  c2 * df_latent$ESCS_z +
...
d1 <- coef_of("SMP", "female")
d2 <- coef_of("SMP", "ESCS_z")
d3 <- coef_of("SMP", "ST001D01T")
d4 <- coef_of("SMP", "MISCED")
d5 <- coef_of("SMP", "SCHSIZE_z")
b  <- coef_of("SMP", "SMS")       # main mediator path
cprime <- coef_of("SMP", "SPI")   # direct effect path

# Intercept for SMP
int_SMP <- pe$est[pe$lhs == "SMP" & pe$op == "~1"]

# Compute predicted SMP
pred_SMP <- int_SMP +
  b       * df_latent$SMS +
