...
fit <- sem(model, data = df, std.lv = TRUE, fixed.x = FALSE, missing = "fiml", estimator = "ML", se = "bootstrap", bootstrap = 100)
...
write_csv(pe,        file.path(DATA_DIR, glue("sem_real_parameterEstimates_{CNT}.csv")))
write_csv(fit_df,    file.path(DATA_DIR, glue("sem_real_fitMeasures_{CNT}.csv")))
write_csv(cor_df, file.path(DATA_DIR, glue("sem_real_latentCorrelations_{CNT}.csv")))
write_csv(rsq_df,    file.path(DATA_DIR, glue("sem_real_rsquare_raw_{CNT}.csv")))
write_csv(real_reliab,           file.path(DATA_DIR, glue("sem_real_reliability_{CNT}.csv")))
...
write_csv(real_fl_df, file.path(DATA_DIR, glue("sem_real_fornell_larcker_{CNT}.csv")))

write_csv(real_paths, file.path(DATA_DIR, glue("sem_real_structural_paths_{CNT}.csv")))

sel_metrics <- c("cfi","tli","rmsea","srmr","chisq","df")
summary_df <- fit_df %>% filter(metric %in% sel_metrics)
write_csv(summary_df, file.path(DATA_DIR, glue("sem_real_fit_summary_{CNT}.csv")))
