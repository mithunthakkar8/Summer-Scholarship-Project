...
OUTPUT_PREFIX = "synthetic_predllm_PV_rep"
...
# -------------------------------------------------
# Construct regression target: Mean PISA Math PV
# -------------------------------------------------
PV_COLS = [f"PV{i}MATH" for i in range(1, 11)]

missing_pv = [c for c in PV_COLS if c not in df.columns]
assert len(missing_pv) == 0, f"Missing PV columns: {missing_pv}"

TARGET_COL = "Y_MATH_MEAN"
df[TARGET_COL] = df[PV_COLS].mean(axis=1)
...
PREDLLM_MODEL_DIR = os.path.join(DATA_DIR, "predllm_distilgpt2_PV_smp")

model_path = Path(PREDLLM_MODEL_DIR)

if model_path.exists():
    print(f"[INFO] Loading existing PredLLM model from {model_path}", flush=True)
    predllm.model.from_pretrained(model_path)
    predllm.tokenizer.from_pretrained(model_path)
else:
